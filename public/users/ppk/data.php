					...
					<div class="page-header-content d-lg-flex">
						...
						<div class="collapse d-lg-block my-lg-auto ms-lg-auto" id="page_header">
							<div class="hstack gap-3 mb-3 mb-lg-0">
                                <a href="<?= base_url("users/ppk/add") ?>" class="btn btn-outline-primary btn-labeled btn-labeled-start rounded-pill">
								...
								<tbody>
									<?php
                                        ...
                                        if(sesuser("id_group") == 3){
                                            $sdata = $this->db->query("SELECT a.*, b.`nm_jk`, c.`nm_agama`, d.`nm_group`, e.nm_opd FROM users a LEFT JOIN jk b ON a.`id_jk` = b.`id_jk` LEFT JOIN agama c ON a.`id_agama` = c.`id_agama` LEFT JOIN `group` d ON a.`id_group` = d.`id_group` LEFT JOIN opd e ON a.id_opd = e.id_opd WHERE a.id_group LIKE '4' AND a.id_opd = '".sesuser("id_opd")."'");
                                        }else{
                                            $sdata = $this->db->query("SELECT a.*, b.`nm_jk`, c.`nm_agama`, d.`nm_group`, e.nm_opd FROM users a LEFT JOIN jk b ON a.`id_jk` = b.`id_jk` LEFT JOIN agama c ON a.`id_agama` = c.`id_agama` LEFT JOIN `group` d ON a.`id_group` = d.`id_group` LEFT JOIN opd e ON a.id_opd = e.id_opd WHERE a.id_group LIKE '4'");
                                        }
                                                        ...
                                                        <div class='d-inline-flex'>
                                                            <div class='dropdown'>
                                                                ...
                                                                <div class='dropdown-menu dropdown-menu-end'>
                                                                    <a href='".base_url("users/ppk/edit/".enkrip($ddata['no_register']))."' class='dropdown-item'>
