<?php
	...
	if(isset($post['cetakin'])){
        $jenis = antixss($post['jenis']);
        $jtk = antixss($post['jtk']);

		if($jenis == "pdf"){
            redirect(base_url("cetak/pdf/lap-jtk/".$jtk));
        }else if($jenis == "excel"){
            redirect(base_url("cetak/excel/lap-jtk/".$jtk));
        }
	}
?><!DOCTYPE html>
                        ...
                        <div class="col-lg-3 col-md-4">
                            <form method="post" class="card" target="_blank">
                                ...
                                <div class="card-body" style="min-height:360px;">
                                    <div class="mb-2">
                                        <label class="form-label">Pilih Jenis Tenaga Kerja :</label>
                                        <select class="form-control select select2-hidden-accessible" data-select2-id="7" tabindex="-1" name="jtk" aria-hidden="true" data-placeholder="Pilih Jenis Tenaga Kerja" required>
											<option></option>
                                            <optgroup label="Pilih Jenis Tenaga Kerja">
                                            <?php
                                                $spen = $this->db->query("SELECT COUNT(jns_keterampilan) AS gelap, jns_keterampilan FROM pekerjaan_ta GROUP BY jns_keterampilan ORDER BY jns_keterampilan");
                                                foreach($spen->result_array() as $dpen){
                                                    echo"<option value='".enkrip($dpen['jns_keterampilan'])."'>".$dpen['jns_keterampilan']."</option>";
                                                }
                                            ?>
                                            </optgroup>
										</select>
                                ...
                                <div class="card-footer">
                                    ...
                                    <button type="submit" name="cetakin" class="btn btn-primary"><i class="ph-printer mx-2"></i> Cetak Laporan</button>
                                </div>
                            </form>
                        </div>
