<?php
	...
	if(isset($post['tambahin'])){
		$post_data	= array(
			"nm_modpilih"	=> antixss($post['nm_modpilih'])
		);
		$hajar		= $this->db->insert("modpilih", $post_data);
		if($hajar){
			$this->session->set_flashdata('pesen', '<script>sukses("tambah");</script>');
		}else{
			$this->session->set_flashdata('pesen', '<script>gagal("tambah");</script>');
		}
		redirect(base_url("master/modpilih"));
	}else if(isset($post['editin'])){
		$post_data	= array(
			"nm_modpilih"	=> antixss($post['nm_modpilih'])
		);
		$hajar			= $this->db->update("modpilih", $post_data, array("id_modpilih" => dekrip($post['id_modpilih'])));
		if($hajar){
		...
		$hajar			= $this->db->delete("modpilih", array("id_modpilih" => dekrip($post['id_modpilih'])));
		if($hajar){
...
<body>
                                                        ...
                                                        <div class='d-inline-flex'>
                                                            <div class='dropdown'>
                                                                ...
                                                                <div class='dropdown-menu dropdown-menu-end'>
                                                                    <a href='#' class='dropdown-item btedit' data-bs-toggle='modal' data-bs-target='#modalEdit' data-id='".enkrip($ddata['id_modpilih'])."' data-nama='".$ddata['nm_modpilih']."'>
                                                                        <i class='ph-pencil me-2'></i>
                                                                        Edit Data
                                                                    </a>
                                                                    <a href='#' class='dropdown-item bthapus' data-bs-toggle='modal' data-bs-target='#modalHapus' data-id='".enkrip($ddata['id_modpilih'])."' data-nama='".$ddata['nm_modpilih']."'>
		...
		<div class="modal-dialog modal-sm">
			<form method="post" class="modal-content">
				...
				<div class="modal-body">
                    <div class="mb-2">
                        <label class="form-label">Nama Metode Pemilihan :</label>
                        <input type="text" name="nm_modpilih" class="form-control" placeholder="Nama Metode Pemilihan" required>
    ...
    <div id="modalEdit" class="modal fade" tabindex="-1">
		<div class="modal-dialog modal-sm">
			<form method="post" class="modal-content">
                <input type="hidden" name="id_modpilih" id="id_modpilih_edit">
				<div class="modal-header">
				...
				<div class="modal-body">
                    <div class="mb-2">
                        <label class="form-label">Nama Metode Pemilihan :</label>
                        <input type="text" name="nm_modpilih" id="nm_modpilih_edit" class="form-control" placeholder="Nama Metode Pemilihan" required>
                        <!-- <span class="form-text">+{3}0 000 000 000</span> -->
                    </div>
				</div>
    ...
    <div id="modalHapus" class="modal fade" tabindex="-1">
		<div class="modal-dialog modal-sm">
			<form method="post" class="modal-content">
                <input type="hidden" name="id_modpilih" id="id_modpilih_hapus">
				<div class="modal-header">
    ...
    <script>
		$(document).ready(function() {
			...
			$(document).on('click', '.btedit', function() {
				const id 	= $(this).data('id');
				const nama 	= $(this).data('nama');
				$('#id_modpilih_edit').val(id);
				$('#nm_modpilih_edit').val(nama);
			});
		});
	</script>
</body>
