<?php
	...
	if(isset($post['tambahin'])){
		$post_data	= array(
			"nm_pekerjaanjns"	=> antixss($post['nm_pekerjaanjns'])
		);
		$hajar		= $this->db->insert("pekerjaanjns", $post_data);
		if($hajar){
			$this->session->set_flashdata('pesen', '<script>sukses("tambah");</script>');
		}else{
			$this->session->set_flashdata('pesen', '<script>gagal("tambah");</script>');
		}
		redirect(base_url("master/pekerjaanjns"));
	}else if(isset($post['editin'])){
		$post_data	= array(
			"nm_pekerjaanjns"	=> antixss($post['nm_pekerjaanjns'])
		);
		$hajar			= $this->db->update("pekerjaanjns", $post_data, array("id_pekerjaanjns" => dekrip($post['id_pekerjaanjns'])));
		if($hajar){
		...
		$hajar			= $this->db->delete("pekerjaanjns", array("id_pekerjaanjns" => dekrip($post['id_pekerjaanjns'])));
		if($hajar){
...
<body>
						...
						<div class="table-responsive">
                            <table class="table table-hover datatable-highlight">
								...
								<tbody>
									<?php
                                        $nodata = 1;
                                        $sdata = $this->db->get("pekerjaanjns");
                                                        ...
                                                        <div class='d-inline-flex'>
                                                            <div class='dropdown'>
                                                                ...
                                                                <div class='dropdown-menu dropdown-menu-end'>
                                                                    <a href='#' class='dropdown-item btedit' data-bs-toggle='modal' data-bs-target='#modalEdit' data-id='".enkrip($ddata['id_pekerjaanjns'])."' data-nama='".$ddata['nm_pekerjaanjns']."'>
                                                                        <i class='ph-pencil me-2'></i>
                                                                        Edit Data
                                                                    </a>
                                                                    <a href='#' class='dropdown-item bthapus' data-bs-toggle='modal' data-bs-target='#modalHapus' data-id='".enkrip($ddata['id_pekerjaanjns'])."' data-nama='".$ddata['nm_pekerjaanjns']."'>
			...
			<form method="post" class="modal-content">
				...
				<div class="modal-body">
                    <div class="mb-2">
                        ...
                        <input type="text" name="nm_pekerjaanjns" class="form-control" placeholder="Nama Jenis Pekerjaan " required>
    ...
    <div id="modalEdit" class="modal fade" tabindex="-1">
		<div class="modal-dialog modal-sm">
			<form method="post" class="modal-content">
                <input type="hidden" name="id_pekerjaanjns" id="id_pekerjaanjns_edit">
				<div class="modal-header">
				...
				<div class="modal-body">
                    <div class="mb-2">
                        <label class="form-label">Nama Jenis Pekerjaan  :</label>
                        <input type="text" name="nm_pekerjaanjns" id="nm_pekerjaanjns_edit" class="form-control" placeholder="Nama Jenis Pekerjaan " required>
                        <!-- <span class="form-text">+{3}0 000 000 000</span> -->
                    </div>
				</div>
    ...
    <div id="modalHapus" class="modal fade" tabindex="-1">
		<div class="modal-dialog modal-sm">
			<form method="post" class="modal-content">
                <input type="hidden" name="id_pekerjaanjns" id="id_pekerjaanjns_hapus">
				<div class="modal-header">
    ...
    <script>
		$(document).ready(function() {
			...
			$(document).on('click', '.btedit', function() {
				const id 	= $(this).data('id');
				const nama 	= $(this).data('nama');
				$('#id_pekerjaanjns_edit').val(id);
				$('#nm_pekerjaanjns_edit').val(nama);
			});
		});
	</script>
</body>
